const onLoadGlobal = () => {
	...
	const pauseGame = () => {
		const textPause = document.querySelector('.pausado');
	...
	const runGame = () => {
		...
		if (window.innerWidth < 400) speed = 1600;
	...
	//Portal animations------------------
	const animRight = () => {
		if (portal.offsetLeft <= gameScreen.offsetWidth - 80) {
	...
	arrowRight.addEventListener('mousedown', () => {
		if (!paused && gameOn && !gameEnd) {
			portalIntervalRight = setInterval(() => animRight(), 6)
		} else return;
	});

	arrowLeft.addEventListener('mousedown', () => {
		if (!paused && gameOn && !gameEnd) {
			portalIntervalLeft = setInterval(() => animLeft(), 6)
		} else return;
	});
	...
	if (navigator.maxTouchPoints !== 0) {
		...
		arrowRight.addEventListener('touchstart', e => {
			...
			if (!paused && gameOn && !gameEnd) {
				portalIntervalRight = setInterval(() => animRight(), 10)
			} else return;
		});
		...
		arrowLeft.addEventListener('touchstart', e => {
			...
			if (Math.abs(timeClick1 - timeClick2) < 300) return;
			if (!paused && gameOn && !gameEnd) {
				portalIntervalLeft = setInterval(() => animLeft(), 10)
			} else return;
		});
